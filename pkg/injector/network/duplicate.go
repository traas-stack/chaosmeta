...
func (i *DuplicateInjector) Inject() error {
	...
	if i.Args.SrcIp == "" && i.Args.DstIp == "" && i.Args.SrcPort == "" && i.Args.DstPort == "" {
		return utils.AddNetemQdisc(i.Args.Interface, "", FaultDuplicate, faultArgs)
	}
	...
	if i.Args.Mode == utils.ModeNormal {
		parent := "1:4"
		if err := utils.AddNetemQdisc(i.Args.Interface, parent, FaultDuplicate, faultArgs); err != nil {
