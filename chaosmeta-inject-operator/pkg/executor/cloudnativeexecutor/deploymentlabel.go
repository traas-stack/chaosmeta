...
func init() {
	registerCloudExecutor(v1alpha1.DeploymentCloudTarget, "label", &DeploymentLabelExecutor{})
}
...
func (e *DeploymentLabelExecutor) Inject(ctx context.Context, injectObject, uid, timeout string, args []v1alpha1.ArgsUnit) (string, error) {
	...
	if deploy.ObjectMeta.Labels != nil {
		backupBytes, err = json.Marshal(deploy.ObjectMeta.Labels)
		if err != nil {
			return "", fmt.Errorf("backup to string error: %s", err.Error())
		}
	}

	newLabels, err := getNewLabels(ctx, deploy.ObjectMeta.Labels, args)
	if err != nil {
		return "", fmt.Errorf("get new labels error: %s", err.Error())
	}

	return string(backupBytes), patchLabels(ctx, c, "deployments", ns, name, newLabels)
}
...
func (e *DeploymentLabelExecutor) Recover(ctx context.Context, injectObject, uid, backup string) error {
	...
	backupBytes, err := getBackupLabels([]byte(backup), deploy.ObjectMeta.Labels)
	if err != nil {
		return fmt.Errorf("get backup labels error: %s", err.Error())
	}

	return patchLabels(ctx, c, "deployments", ns, name, backupBytes)
}
