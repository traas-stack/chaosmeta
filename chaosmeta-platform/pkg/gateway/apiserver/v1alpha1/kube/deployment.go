...
func (c *KubeController) ListDeployments() {
	...
	ns := kube.NewDeploymentService(&kubernetesService.KubernetesParam{KubernetesClient: kubeClient, RestConfig: restConfig})
	resp, err := ns.List(nsName, metav1.ListOptions{}, page.ParseDataSelectPathParameter(&c.Controller))
	if err != nil {
		c.Error(&c.Controller, err)
		return
	}
