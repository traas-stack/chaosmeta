...
func initMeasureCommon(ctx context.Context, injectId int, measureType string) error {
	argMeasureType := basic.Args{InjectId: injectId, ExecType: ExecMeasureCommon, Key: "measureType", KeyCn: "measureType", ValueType: "string", ValueRule: measureType, Required: true, DescriptionCn: "度量操作类型", Description: "Metric operation type"}
	argDuration := basic.Args{InjectId: injectId, ExecType: ExecMeasureCommon, Key: "duration", KeyCn: "持续时长", ValueType: "string", Unit: "s", UnitCn: "s", Required: true, DescriptionCn: "度量任务持续时间", Description: "Measure task duration"}
	argsInterval := basic.Args{InjectId: injectId, ExecType: ExecMeasureCommon, Key: "interval", KeyCn: "度量操作执行间隔", ValueType: "string", Unit: "s,m,h", UnitCn: "s,m,h", Required: true, DescriptionCn: "度量操作执行间隔", Description: "Measuring operation execution interval"}
	argsSuccessCount := basic.Args{InjectId: injectId, ExecType: ExecMeasureCommon, Key: "successCount", KeyCn: "成功次数阈值", ValueType: "int", ValueRule: ">0", DescriptionCn: "成功次数阈值,度量任务结束时,度量成功次数不小于successCount,则CR任务状态为success", Description: "Threshold of the number of successes. When the measurement task ends, if the number of successes is not less than successCount, then the CR task status is success."}
	argsFailedCount := basic.Args{InjectId: injectId, ExecType: ExecMeasureCommon, Key: "failedCount", KeyCn: "失败次数阈值", ValueType: "int", ValueRule: ">=0", DescriptionCn: "失败次数阈值,度量任务结束时,度量失败次数不小于failedCount，则CR任务状态为failed", Description: "Failure count threshold. When the measurement task ends, if the number of measurement failures is not less than failedCount, then the CR task status is failed."}
...
func InitMonitorMeasure(ctx context.Context) error {
	var (
		monitorMeasure = basic.MeasureInject{MeasureType: "monitor", Name: "monitor", NameCn: "监控度量", Description: "Make expected judgments on the values of monitoring items, such as whether the CPU usage monitoring value of a certain machine is greater than 90%. Prometheus is supported by default", DescriptionCn: "对监控项的值进行预期判断,比如某个机器的cpu使用率监控值是否大于90%,默认支持prometheus"}
	)
...
func InitTcpMeasure(ctx context.Context) error {
	var (
		tcpMeasure = basic.MeasureInject{MeasureType: "tcp", Name: "tcp", NameCn: "TCP度量", Description: "Make expected judgments on TCP requests, such as testing whether the 8080 port of a certain server can be passed", DescriptionCn: "对tcp请求进行预期判断，比如测试某个服务器的8080端口是否能通"}
	)
...
func initTcpMeasureJudge(ctx context.Context, measureInject basic.MeasureInject) error {
	argsJudgeType := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasureCommon, Key: "judgeType", KeyCn: "预期判断方式", ValueType: "string", ValueRule: "connectivity", Required: true, DescriptionCn: "判断目标端口tcp服务是否能连通", Description: "Determine whether the target port tcp service can be connected"}
	argsJudgeValue := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasureCommon, Key: "judgeValue", KeyCn: "预期判断值", ValueType: "string", ValueRule: "true,false", Required: true, DescriptionCn: "预期判断值", Description: "expected judgment value"}
	return basic.InsertArgsMulti(ctx, []*basic.Args{&argsJudgeType, &argsJudgeValue})
}
...
func InitTcpMeasureArgs(ctx context.Context, measureInject basic.MeasureInject) error {
	...
	argsIp := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasure, Key: "ip", KeyCn: "目标机器", ValueType: "string", DescriptionCn: "IP,比如:192.168.2.189", Description: "IP,for example: 192.168.2.189"}
...
func InitPodMeasure(ctx context.Context) error {
	var (
		podMeasure = basic.MeasureInject{MeasureType: "pod", Name: "pod", NameCn: "POD度量", Description: "Make expected judgments on pod-related data, such as whether the number of pod instances of an application is greater than 3", DescriptionCn: "对pod相关数据进行预期判断,比如某个应用的pod实例数是否大于3"}
	)
...
func initPodMeasureJudge(ctx context.Context, measureInject basic.MeasureInject) error {
	argsJudgeType := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasureCommon, Key: "judgeType", KeyCn: "预期判断方式", ValueType: "string", ValueRule: "count", Required: true, DescriptionCn: "判断pod的数量是否符合预期\njudgeValue样例：\n\"2,5\"：2<=pod数量<=5\n\"5\"：pod数量等于5", Description: "Determine whether the number of pods is as expected\njudgeValue example:\n\"2,5\": 2<=number of pods<=5\n\"5\": The number of pods is equal to 5"}
	argsJudgeValue := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasureCommon, Key: "judgeValue", KeyCn: "预期判断值", ValueType: "string", Required: true, DescriptionCn: "预期判断值", Description: "expected judgment value"}
	return basic.InsertArgsMulti(ctx, []*basic.Args{&argsJudgeType, &argsJudgeValue})
}
...
func InitPodMeasureArgs(ctx context.Context, measureInject basic.MeasureInject) error {
	...
	argsNamespace := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasure, Key: "namespace", KeyCn: "k8s命名空间", ValueType: "string", DescriptionCn: "集群中已存在的namespace", Description: "Namespace that already exists in the cluster"}
	argsLabel := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasure, Key: "label", KeyCn: "k8s标签", ValueType: "string", DescriptionCn: "k8s标签", Description: "K8S label"}
...
func InitHTTPMeasure(ctx context.Context) error {
	var (
		httpMeasure = basic.MeasureInject{MeasureType: "http", Name: "http", NameCn: "HTTP度量", Description: "Make expected judgments on http requests, such as whether the return status code is 200 when making a specified http request", DescriptionCn: "对http请求进行预期判断,比如进行指定的http请求时,返回状态码是否为200"}
	)
...
func InitHTTPMeasureArgs(ctx context.Context, measureInject basic.MeasureInject) error {
	...
	argsHost := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasure, Key: "host", KeyCn: "目标机器", ValueType: "string", DescriptionCn: "IP或域名", Description: "IP or domain name"}
	argsPort := basic.Args{InjectId: measureInject.Id, ExecType: ExecMeasure, Key: "port", KeyCn: "目标机器端口", ValueType: "string", DescriptionCn: "单个端口号", Description: "Single port number"}
	...
	return basic.InsertArgsMulti(ctx, []*basic.Args{&argsHost, &argsPort, &argsPath, &argsHeader, &argsScheme, &argsMethod, &argsTimeout, &argsBody})
}
